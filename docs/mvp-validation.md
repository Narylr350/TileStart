...
| Win10 对照证据 | 原版截图 `native-start-overview-2560x1600-150pct.png` 为 2560×1600；现有 TileStart 文件 `tilestart-mvp-2560x1600-150pct.png` 实际为 1707×1067，不能作为同物理分辨率像素对照。后续必须重新采集同尺度截图后才能验收视觉一致性 |

## 当前开放项

以下项目没有足够源码、反编译或实机证据，不能标记为完成：

1. 重新采集同物理尺度的原版/TileStart 静态截图，先锁定窗口、导航轨、应用列表、磁贴区和分组网格的几何锚点。
2. 完成背景材质、字体、高清图标资源、应用列表密度、磁贴内容模板、文字基线、导航轨和滚动条等静态视觉校准。
3. 按原版 `SemanticZoom` 结构恢复字母索引；原版菜单模式使用 48 DIP 单元、20 DIP 字号、4 列布局且没有“选择字母”标题或独立卡片边框。
4. 静态截图验收通过后，再按 `docs/win10-start-motion-reverse.md` 继续 AppLaunch、ViewSwitch 等尚未完成的 Motion，并恢复拖动 120 ms reflow、3 DIP 抖动过滤和让位动画。
5. 采集并恢复导航轨展开、应用文件夹、上下文菜单、磁贴 hover/press/release 等状态和动画。
6. 使用物理键盘逐项确认单独 `Win` 和 `Win+E/R/D/L/I/数字/方向键/Shift+S`。
7. 使用鼠标实际操作通知区域菜单，确认暂停、恢复和打开原版开始菜单的桌面体验。
8. 在 100%、125%、175%、200% DPI 环境验证窗口、菜单和设置窗口。
9. 在多显示器及混合 DPI 环境验证触发显示器选择和副任务栏定位。
10. 验证锁屏、注销、重新登录和登录自启动后的完整生命周期。
11. 验证全屏游戏或独占全屏应用期间不会误弹。
12. 在 Windows 11 对目标 build 建立独立任务栏适配器并完成实机验证；当前仅允许 build 19045 注入。
