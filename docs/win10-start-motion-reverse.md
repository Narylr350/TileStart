...
2026-07-22 重新录制了当前 Win10 原版 SemanticZoom 双向切换：`2560×1600`、`120 FPS`、时长 `7.018 s`、文件大小 `8,771,657 bytes`、SHA-256 `F46E4656448B21CF6EEC4105C8E343D477C33334D298BF43DB873F6E2FD6F18F`。录像本身保存在仓库外；分析时按 60 FPS 生成带帧号联系表，再由两个独立视觉分析任务分别估算进入和返回方向的运动进度、透明度与软化区间。

帧级结果表明两个方向不应再共用一个长时 ease-out：进入索引在约 `17 ms` 预淡化后执行约 `217 ms` 几何运动，完整清晰度收尾约 `250 ms`；返回列表的几何运动约 `183 ms`，完整收尾约 `200 ms`。首次使用单个 cubic-bezier 拟合这些采样点，但后续项目录像证明单一 spline 会在前中段过早接近终点，虽然属性 duration 正确，肉眼可见的有效几何仍分别缩短到约 `133–150 ms` 和 `100–117 ms`。

用于对照的 TileStart 录像同样录于 2026-07-22：`2560×1600`、`120 FPS`、时长 `4.778 s`、文件大小 `5,966,294 bytes`、SHA-256 `0B7C8890732E7FB9C4C33374E5BB791399801850BFA9A583BFF3DA398F73116A`。原版与项目联系表的独立对比确认共享中心、Scale 起止值、Translate 方向和 `0.5` presenter 补偿均正确，偏差集中在同一路径上的进度分配、incoming presenter 可见度和清晰度恢复时机。

当前几何动画直接使用原版帧进度约束的分段关键帧，不再继续猜单个 spline。进入索引的归一化 `(time, progress)` 为 `(0,0)`、`(0.15,0.10)`、`(0.46,0.54)`、`(0.77,0.92)`、`(1,1)`；返回列表为 `(0,0)`、`(0.20,0.18)`、`(0.50,0.62)`、`(0.80,0.94)`、`(1,1)`。所有共享 Scale/Translate 属性使用同一组关键帧，保持几何中心不变。

原版中间帧的不可辨识度由高速缩放、纹理重采样、两层透明度重叠和可能的运动软化共同造成，不能仅凭录屏证明存在独立 motion-blur shader。项目对比还表明同时给两个 presenter 施加同等 Gaussian blur 会产生滤镜感，且无法隐藏彩色图标中心。当前峰值半径降为 `5`：incoming presenter 使用完整钟形包络，outgoing presenter 只使用 `0.35×`；返回列表时 incoming 应用层采用前慢后快的分段 Opacity，使其到几何中点仍只有约 `0.35`，最后约 `8%` 才达到完全可辨识。动画完成后移除 Effect 与临时 `BitmapCache`。
