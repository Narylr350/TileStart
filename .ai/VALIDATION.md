...
- 扫描后处理原先会为隐藏过滤和自定义应用去重重复调用 `LaunchTargetIdentity.GetKey`；当前 333 个应用、1 个隐藏身份、1 个自定义应用的配置中，最坏接近 999 次身份规范化，而每个 `.lnk` 调用都会单独创建和释放 WScript Shell COM。后处理现使用 operation-local 原始目标缓存和单线程、单次扫描范围内的惰性 WScript Shell 会话，并通过 `ConfigureAwait(false)` 离开 UI Dispatcher；不做跨线程或跨启动 COM 缓存。真实 Release 连续三次从 `Application scan completed` 到 `Application content ready` 为 519、520、511 ms，旧路径为 1339–1609 ms，另一次高竞争采样为 2330 ms；同期初始磁贴视觉批次稳定为 1129–1198 ms。
- `ShellIconLoader.Load` 保留单次调用兼容行为，但应用图标组在各自的单线程批次内复用一个惰性 WScript Shell 会话；会话不跨线程、不跨批次，并在组结束时释放。285 个经典应用的真实 Release 三次耗时由 3398、3432、3445 ms 降到 3245、3188、3135 ms，平均约快 7%；48 个 Packaged 应用仍为 104–109 ms，应用内容就绪和初始磁贴视觉未退化。
- 完整磁贴视觉批次现同样在自身 STA 线程内复用一个 `LaunchTargetIdentity.Resolver` 和一个 `ShellIconLoader.Session`，贯穿应用身份索引、顶层磁贴和文件夹子磁贴；`RestoreTileIcon` 等单磁贴交互路径仍使用单次解析。真实 Release 三次初始本地视觉由 1116、1188、1169 ms 降到 1106、1159、1127 ms，完整应用视觉由 846、793、760 ms 降到 746、741、715 ms，后者平均约快 8%。
...
- 首次右键真实 ChatGPT 磁贴时，从按下到 `ContextMenu.Opened` 曾约为 194 ms，其中菜单动态资格判断和动画合计约 15 ms，主要成本位于 WPF 为 `x:Shared=False` 的磁贴菜单首次创建独立 Popup 视觉树。磁贴现共享一个 `ContextMenu`，并在隐藏启动阶段跨过 `DispatcherPriority.Loaded` 真正预热 Popup 与首个子菜单；应用列表菜单则必须等待完整磁贴视觉和应用图标任务完成后再预热。禁止在 `Application content ready` 后立即重复预热全部菜单：真实采样曾与图标应用阶段重叠 949.58 ms，造成首屏图标延后。用户实机确认首次右键明显改善，且将应用菜单预热移出视觉关键路径后图标出现速度恢复可接受。
- 主题切换重启或冷启动时，Host 可能在 Shell 应用扫描完成前收到 `Open` 请求。所有主窗口打开入口必须等待 `Application content ready`，并把就绪前的多次请求合并为一次待显示操作；不得先展示仅含同步恢复磁贴、但应用列表和字母索引尚未完成的半成品窗口。
