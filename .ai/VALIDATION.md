...
- 95 个磁贴、332–333 个应用时，`Application content ready` 到完整磁贴视觉恢复曾耗时 34.7–55.9 秒。相同真实布局的一次性基准中，现有线性身份匹配耗时 56.70 秒、调用 `LaunchTargetIdentity.GetKey` 22588 次；按应用身份建立一次局部字典后耗时 0.99 秒、调用 427 次，约快 57 倍。实现时必须保留应用扫描顺序中的第一个重复身份，并让文件夹子磁贴复用同一索引；完成后在相同数据规模下复测耗时和调用次数。
...
- 不能简单删除初始视觉树预热：取消后 Host 启动约从 1.07 秒降到 0.65 秒，但第一次打开从约 217 ms 增到约 723 ms。实验性改为 `DispatcherPriority.ApplicationIdle` 延后预热时，Host 可先在约 0.65 秒完成，预热完成后的第一次打开为 164–244 ms；打开请求恰好落在预热执行期间时，从请求到 `Window.Show` 约 423 ms。后续实现应采用可取消或可让高优先级打开请求先行的空闲预热，而不是把 0.5 秒工作直接挪到首次交互。
