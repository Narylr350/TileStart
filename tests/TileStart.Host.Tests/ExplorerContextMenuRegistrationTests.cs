...
public sealed class ExplorerContextMenuRegistrationTests
{
    ...
    [Fact]
    public void ExplorerCommandsUseConsistentTileStartTerminology()
    {
        Assert.Equal("添加到 TileStart 应用列表", ExplorerContextMenuRegistration.AddToAppListLabel);
        Assert.Equal("固定到 TileStart 磁贴区", ExplorerContextMenuRegistration.PinToStartLabel);
        Assert.DoesNotContain("“开始”屏幕", ExplorerContextMenuRegistration.PinToStartLabel);
    }
