...
$symbolManifest = Join-Path $env:TEMP 'TileStart\reverse\startui-icon-deep-evidence.txt.manifest.json'
if (-not (Test-Path -LiteralPath $symbolManifest)) { $symbolManifest = Join-Path $env:TEMP 'TileStart\reverse\startui-evidence.txt.manifest.json' }
