...
namespace
{
    ...
    std::optional<DWORD> GetWindowsBuildNumber()
    {
        ...
        const auto rtl_get_version = reinterpret_cast<LONG(WINAPI*)(OSVERSIONINFOW*)>(GetProcAddress(
            ntdll, "RtlGetVersion"));
    ...
    std::optional<std::uintptr_t> FindRemoteExport(DWORD process_id, const std::wstring& dll_path,
    ...
    bool LoadRemoteLibrary(HANDLE process, const std::wstring& dll_path)
    {
        ...
        const LPVOID remote_path = VirtualAllocEx(process, nullptr, byte_count, MEM_COMMIT | MEM_RESERVE,
                                                  PAGE_READWRITE);
        ...
        const auto result = copied && written == byte_count
                                ? CallRemoteExport(process, reinterpret_cast<std::uintptr_t>(load_library), remote_path)
                                : std::nullopt;
    ...
    int Probe(const std::filesystem::path& dll_path)
    {
        ...
        std::wcout << (acknowledged
                           ...
                           : std::format(
                               L"Host unavailable or timed out (error={}); native Start must remain allowed.\n",
                               error));
