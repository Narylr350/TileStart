...
internal static class LaunchTargetIdentity
{
    public static string GetKey(string launchTarget)
    {
        ...
        return resolver.GetKey(launchTarget);
    }

    public static Resolver CreateResolver() => new();
    ...
    internal sealed class Resolver : IDisposable
    {
        ...
        public string GetKey(string launchTarget)
        {
        ...
        private string? ResolveShortcutTarget(string path)
        {
            if (!File.Exists(path)
